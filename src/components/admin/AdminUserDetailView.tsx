        ...
        <AdminSection title="Trust & safety">
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-white/70">Reports filed</dt>
              <dd className="font-semibold">{user.reportsFiled}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-white/70">Reports against user</dt>
  ...
  const reviewsPanel = (
    <AdminScrollPanel maxHeight="calc(100dvh - 16rem)">
      {user.recentReviews.length === 0 ? (
        <p className="p-4 text-sm text-white/70">No reviews yet.</p>
      ) : (
          ...
          <tbody>
            {user.recentReviews.map((review) => (
              <AdminTableRow key={review.id}>
                <AdminTableCell>
                  <Link href={`/reviews/${review.id}`} className="font-medium text-[#6e46c7] hover:underline">
                    {review.title}
                  </Link>
                </AdminTableCell>
                <AdminTableCell>
  ...
  const auditPanel = (
    <AdminScrollPanel maxHeight="calc(100dvh - 16rem)">
      {user.recentAudit.length === 0 ? (
        <p className="p-4 text-sm text-white/70">No related audit events.</p>
      ) : (
